...
Memory is a pure sibling assembly with one-way references to Core, Ethics, and
Emotion. Core does not reference Memory. There is no `SLASHBridge`, Unity
lifecycle method, wall clock, random source, static global state, or mutable
ScriptableObject runtime state in this layer.
...
- copied Ethics evaluation identities and complete traces; and
- the resulting Emotion position, velocity, integrated velocity, net
  influence, unchanged-position duration, and emotional swing.
...
state it is claimed to have influenced.
...
### Three facts, not three competing truths

Action outcome, Ethics, and Emotion answer different questions:

- the Core lifecycle says what the Executable actually did;
- Ethics records how that event was evaluated in its supplied context; and
- Emotion records the state and motion that evaluation helped produce.
...
Fading never erases the episode's factual lifecycle outcome, Ethics trace, or
produced Emotion consequence. Every episode has already contributed to the
cluster's aggregate projector frequencies, outcome counts, emotional sums,
first/last Tick, repetition count, and trauma count.
...
`Restore`. This first slice supplies exact continuation state and a rehydration
boundary, not a built-in general file codec.
